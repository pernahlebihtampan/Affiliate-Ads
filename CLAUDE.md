...
- **`ImportBatch`**: menyimpan `fileModifiedTime`/`fileSize` (dikirim eksplisit oleh halaman import — multipart tidak membawa `lastModified`). Import ditolak bila file **sama/lebih lawas** dari import terakhir per `(type, accountId)` (`checkFileIsNewer` di import-service), selain tolakan duplikat via `fileHash`. **Dapat dihapus** lewat `DELETE /api/import/history` dari UI `/riwayat-impor`, **hanya batch terkini per `(type, accountId)`** (guard cari batch max `importedAt`/`id` di grup; else `409`) — menghapus batch lama tak aman karena barisnya mungkin sudah di-update batch lebih baru. Type→tabel: `meta`→`MetaAdDaily`, `meta_placement`→`MetaAdPlacement`, `shopee_click`→`ShopeeClick`, `shopee_commission`→`ShopeeOrderItem`. Delete (dalam satu transaksi): hapus baris `firstImportId == id`, kosongkan `lastImportId` baris yang cuma di-update (buang pointer menggantung), hapus baris tabel riwayat `importBatchId == id` (`meta`→`MetaAdDailyHistory`, `shopee_commission`→`ShopeeOrderItemHistory`), lalu hapus batch. Setelah transaksi, bila batch ber-`reportDate`: snapshot tanggal itu dihitung ulang (batch same-date sebelumnya otomatis naik jadi sumber) atau dihapus bila tak ada batch tersisa. Dimensi `ShopeeCampaign`/`MetaCampaign` **tidak** dihapus (dipakai bersama & ditaut `CampaignHub`).

- **`MetaAdDailyHistory` / `ShopeeOrderItemHistory` + `DailySnapshot`** (Dasbor Harian, tab **Laporan** & **Rentang** di `/`): tab ini merender payload JSON beku `DailySnapshot` per `reportDate` (`/api/daily`, `/api/daily/range`), dihitung `computeReportForBatchIds` (`src/lib/daily-snapshot.ts`) di akhir tiap impor ber-`reportDate` & tombol "Hitung ulang". Sumber fakta = **tabel riwayat insert-only per-batch** (kolom ramping — hanya yang dibaca laporan), ditulis `importMetaAdCsv`/`importShopeeCommissionCsv` **hanya saat `reportDate` ada** (Map dedup kunci-unik, flush `createMany` per 500; `createMany` SQLite tak punya `skipDuplicates`). Baris riwayat **tak pernah di-update** → hitung-ulang tanggal lama selalu mereproduksi isi file saat itu, kebal upsert impor berikutnya. Klik Shopee tanpa tabel riwayat: atribusi via `firstImportId` (insert-only, tak pernah berubah). Status "Penayangan" dibekukan dari `delivery` baris riwayat bertanggal terbaru (bukan `MetaCampaign.status` hidup). **Guard dobel-hitung**: bila `(type, accountId)` sama punya beberapa batch se-`reportDate` (file koreksi), hanya batch **terbaru** yang dipakai (batch klik selalu semua — `klikId` unik). **Fallback legacy**: batch pra-tabel-riwayat (0 baris riwayat) dihitung dengan atribusi `lastImportId` lama (`computeReportForBatchIdsLegacy`) agar recompute tanggal lama tidak menimpa payload bagus dengan laporan kosong. Riwayat Dibatalkan & organik (`shopeeCampaignId` null) **disimpan**, difilter saat baca.
